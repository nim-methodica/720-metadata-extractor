...
## שדה 6: `contentType` (3 קטגוריות)

**חוק פשוט** לפי תפקיד הפריט:

| סימני זיהוי | contentType |
|---|---|
| פריט הקנייה — סרטון/קלפים/פלייליסט, מציג מושג חדש | `Instruction` |
| פריט הוק / בחירת דמות / פתיח מוטיבציוני | `Instruction` |
| פריט העשרה (סרטון בלי שאלה מוערכת) | `Instruction` |
| תרגול מונחה — פתרון שלב-שלב יחד עם הדמות (בעיקר במתמטיקה) | `Solved Exercise` |
| כל פריט אחר עם שאלה — חימום/בסיסי/סטנדרטי/מתקדם/משימת כיתה/שאלת שיא | `Practice` |
...
- סעיף = שאלה יחידה ב-`questions[]`.
- פריט ללא שאלה מוערכת (הקנייה טהורה, סרטון העשרה) → הערך 1 דקה לפריט.
- פריט משימת כיתה → הערך 2 דקות (סעיף אחד).
...
**חוק**: `title` של פריט = `<סוג התרגיל> <מספר בסוג>: <תיאור קצר מהשקף>` — למשל:
...
תבנית הפירוט הקצר — מהשקף הראשון של הפריט (הכותרת או הנושא המרכזי).
